...
void SegmentQLineSeries::append(QPointF point) {
	...
	qDebug() << point;
	if (numberOfPoints != 0) {
		QLineSeries* lastSegment = segments.last();
	...
	if (currentSegmentType != lastSegmentType) {
		...
		/*if (lastSegmentType = INCREASING) {
			indexOfIncreasingSegments.append(segments.size() - 1);
			currentSegment->setPen(QPen(increasingSegmentColor));
		}
		else {
			indexOfDecreasingSegments.append(segments.size() - 1);
			currentSegment->setPen(QPen(decreasingSegmentColor));
		}*/
...
void SegmentQLineSeries::setWidth(int width) {
	width = width;
...
void SegmentQLineSeries::setColor(QColor color, SegmentType segmentType) {
	/*QVector<int>& indexOfSegments = segmentType == INCREASING ? indexOfIncreasingSegments : indexOfDecreasingSegments;

	if (segmentType == INCREASING) {
		increasingSegmentColor = color;
	}
	else {
		decreasingSegmentColor = color;
	}*/
